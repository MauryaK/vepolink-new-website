...
// ─── Leadership ──────────────────────────────────────
const team = [
  { n: "Dr. Arvind Krishnan", r: "Co-founder & CEO", b: "Twelve years at Tata Projects, leading environmental engineering for thermal & process industries.", li: "in/arvind-krishnan" },
  { n: "Priya Venkatesh", r: "Co-founder & CTO", b: "Built the original DAS firmware. Previously embedded systems lead at Ather Energy. IIT Madras alum.", li: "in/priya-venkatesh" },
  { n: "Rohan Mehta", r: "VP Engineering", b: "Cloud architect — designed Vepolink's serverless ingest layer handling 38M+ events daily.", li: "in/rohan-mehta" },
  { n: "Dr. Anjali Rao", r: "Head of Environmental Science", b: "PhD in atmospheric chemistry, CSIR-NEERI. Leads our calibration and EN 14181 compliance frameworks.", li: "in/anjali-rao" },
];
      ...
      <Reveal>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 64, marginBottom: 56 }}>
          <div>
            ...
            <h2 style={{ fontSize: 44, lineHeight: 1.02, letterSpacing: "-0.025em", margin: 0, fontWeight: 500 }}>
              Operators<br/>
              <span className="serif-i">and instrument-makers.</span>
            </h2>
          </div>
          <p style={{ fontSize: 17, lineHeight: 1.5, color: "var(--mute)", margin: 0, alignSelf: "end", maxWidth: 580, textWrap: "pretty" }}>
            The people building Vepolink have spent careers in plant rooms,
            analyser labs and cloud control planes. They've felt the gap they're
            closing.
          </p>
        </div>
      </Reveal>
...
const TeamCard = ({ n, r, b, li, idx }) => {
  const initials = n.split(" ").map(w => w.replace(".", "")[0]).filter(Boolean).slice(0, 2).join("");
  // Subtle differentiation: portrait placeholder with gradient + initials
  const grads = [
    "linear-gradient(135deg, oklch(58% 0.21 290 / 0.18), oklch(78% 0.13 210 / 0.18))",
    "linear-gradient(135deg, oklch(72% 0.13 60 / 0.14), oklch(65% 0.14 160 / 0.16))",
    "linear-gradient(135deg, oklch(64% 0.20 25 / 0.14), oklch(58% 0.21 290 / 0.14))",
    "linear-gradient(135deg, oklch(65% 0.13 155 / 0.16), oklch(78% 0.13 210 / 0.16))",
  ];
  return (
    <article style={{
      background: "var(--card)", border: "1px solid var(--line)", borderRadius: 6,
      ...
      <div style={{
        height: 220, background: grads[idx % grads.length],
        borderRadius: 4, position: "relative",
        border: "1px solid var(--line-2)",
        display: "flex", alignItems: "center", justifyContent: "center",
      }}>
        <span style={{
          fontFamily: "Geist", fontSize: 64, fontWeight: 500,
      ...
      <div style={{ paddingTop: 18, display: "flex", flexDirection: "column", flex: 1 }}>
        ...
        <p style={{ fontSize: 13.5, color: "var(--mute)", lineHeight: 1.55, margin: "12px 0 0", textWrap: "pretty" }}>
          {b}
        </p>
        <a href={`https://linkedin.com/${li}`} style={{
          marginTop: "auto", paddingTop: 14,
          display: "inline-flex", alignItems: "center", gap: 8,
          fontSize: 12.5, color: "var(--ink)", textDecoration: "none",
          fontFamily: "Geist Mono", letterSpacing: "0.04em",
        }}>
          LinkedIn <Icon name="arrowUp" size={12} />
        </a>
      </div>
    </article>
  );
